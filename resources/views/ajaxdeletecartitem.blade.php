<div class="content-right">
  ...
  <div class="content-right-center" id="cardData">
      ...
      @if (session('cart'))
          ...
          @foreach (session('cart') as $id => $details)
              @php
                  $total += $details['price'];
                  $duetotal = number_format($total, 2);
              ...
              <div class="order-item" id="orderItem_{{ $id }}">
                  ...
                  <div class="order-item-detail">
                      <p id="pname">{{ Str::limit($details['name'], 8) }}
                          <br>
                          <span style="opacity: 1; color:black"
                              id="discount_type_{{ $id }}">{{ $details['discount_name'] }}</span>
                          <br>
                          <span style="opacity: 1; color:black border:black"
                              id="note_prod_{{ $id }}">{{ $details['notes'] }}</span>
                      ...
                      <p>
                          ...
                          <br>
                          <span style="opacity: 1; color:black;"
                              id="discount_total_{{ $id }}">{{ $details['discount_value'] != '' ? "-$" . $details['discount_value'] : '' }}</span>
                      </p>
                  </div>
                  <div class="order-item-edit">
                      <span>

                          <button type="button" class="btn-styleless" id="popupBtn" data-bs-toggle="modal"
                              data-bs-target="#exampleModal"
                              onclick="popup({{ $id }}, {{ $details['price'] }})">
                      ...
                      <button type="button" class="btn-styleless"
                          onclick="plus({{ $details['quantity'] }},{{ $id }})" id="plus"
                      ...
                      <button type="button" class="btn-styleless"
                          onclick=" minus({{ $details['quantity'] }},{{ $id }})" id="minus"
                          class="changeQuantity">
                          <svg width="18" height="19" viewBox="0 0 18 19" fill="none"
                              xmlns="http://www.w3.org/2000/svg">
                              <path
                      ...
                      <span>
                          <button type="button" class="btn-styleless" id="popupBtnNote" data-bs-toggle="modal"
               ...
               <div class="modal-dialog modal-dialog-centered">
                 ...
                 <div class="modal-content">
                   ...
                   <div class="d-flex justify-content-center align-content-center gap-2">
                     <button type="button" data-dismiss="modal" aria-hidden="true" class="btn-orang px-5 py-3" onclick="notes({{$id}})">APPLY Note</button>
                   </div>
                   </div>
          ...
          @endforeach
      @else
          <div class="m-auto text-center" id="dish">
              <span>
  ...
  @if (session('cart'))
      ...
      @foreach (session('cart') as $key => $disc)
          @php
              $totaldisc += intval($disc['discount_value']);
              $totaldiscount = number_format($totaldisc, 2);
          @endphp
      @endforeach
      ...
      <div class="content-right-footer-btn">
          ...
          @if (session('cart'))
              <button type="button" id="orderN" class="footer-btn" style="background: var(--orang-2)"
                  onclick="order({{ $subtotal }},{{ $totaldiscount }},{{ $duetotal }})">
                  <span>
                      <svg width="18" height="19" viewBox="0 0 18 19" fill="none"
                          xmlns="http://www.w3.org/2000/svg">
                          <path
